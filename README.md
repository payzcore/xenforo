...
| **Monitoring Expiry** | Minutes before monitoring expires | `60` |
...
2. The add-on creates a monitoring request via the PayzCore API
3. User sees a payment page with:
...
4. User sends the stablecoin from their wallet to the displayed address
5. PayzCore detects the incoming transfer on the blockchain
6. PayzCore sends a signed webhook to your XenForo callback URL
7. The add-on verifies the signature and activates the user upgrade
...
└── _output/
    ├── phrases/                        # Language strings (30+ phrases)
    └── templates/
        ├── admin/
